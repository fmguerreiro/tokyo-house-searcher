...
(defn scrape-listings [url]
  (let [names  (scrape url *name-selector*)
    ...
    (->> (interleave names prices sizes dists)
         (partition 4)
         (map #(into {} (mapv hash-map '(:name :price :size :distance) %))))))
...
#_(
   ...
   (scrape url *size-selector*)
   (def ^:dynamic *name-selector* [:h2.property_inner-title :.js-cassetLinkHref :> html/text-node])
   (scrape url *name-selector*)
   ...
   (scrape-listings url)
   (into {} (mapv hash-map '(:name :price :size :distance) '("a" "b" "c" "d")))
